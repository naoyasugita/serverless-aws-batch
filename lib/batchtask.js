...
function compileBatchTask(functionName) {
  ...
  return generateLambdaScheduleArtifact.bind(this)(functionName).then(() =>{
    ...
    _.unset(this.serverless.service.functions, `${functionName}.iam.role.statements`)
...
function generateBatchJobExecutionRole(functionName, functionObject) {
  ...
  // Merge our iam.role.Statements into this role
          ...
          "Policies": [
            {
              ...
              "PolicyDocument": {
                "Version": "2012-10-17",
                "Statement": this.serverless.service.provider.iam.role.statements
              }
            }
          ]
        }
  ...
  // Merge in custom iam.role.Statements from lambda function definition
  if (functionObject.iam?.role?.statements != undefined) {
    _.mergeWith(
      batchJobExecutionRole.Properties,
      {
        "Policies": [
          {
            ...
            "PolicyDocument": {
              "Version": "2012-10-17",
              "Statement": functionObject.iam.role.statements
            }
          }
        ]
      },
